...
public class UserWebUnitTest {
    ...
    public void findAll_requestAllUsers_returnsAllUsers() throws Exception {
        ...
        set2.add(new Authority().setName("CAN_RETRIEVE_ALL_PRODUCTS"));
        Set<Role> set3 = new HashSet<>();
        Set<Authority> set4 = new HashSet<>();
        set4.add(new Authority().setName("CAN_DELETE_USER"));
        set1.add(new Role(UUID.randomUUID(), "ADMIN", null, null).setAuthorities(set2));
        set3.add(new Role(UUID.randomUUID(), "CUSTOMER", null, null).setAuthorities(set4));
        User user1 = new User(UUID.randomUUID(), "cs@mail.com", "", "Chris", "South", set1, null, null);
        User user2 = new User(UUID.randomUUID(), "aa@mail.com", "", "Anna", "Abby", set3, null, null);
        ...
        mockMvc.perform(
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$[*].surname").value(Matchers.containsInAnyOrder(user1.getSurname(), user2.getSurname())))
